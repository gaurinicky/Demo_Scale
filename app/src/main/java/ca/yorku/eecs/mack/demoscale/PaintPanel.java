...
public class PaintPanel extends View
{
    ...
    private void initialize(Context context)
    {
        ...
        targetImage = context.getResources().getDrawable(R.drawable.berg);
        ...
        zoomTimer = new CountDownTimer(TIMER_DELAY, TIMER_DELAY) {
            @Override
            public void onTick(long millisUntilFinished) {
    ...
    private void doZoom()
    {
        ...
        if(zoomIn)
        {
            zoomIn = false;
            scaleFactor /=  3;
            //when zoomed in we want to decrease the image on the next double tap
            xPosition = xPosition - xRatio / imageIntrinsicWidth * (scaleFactor/10);
            yPosition = yPosition - yRatio / imageIntrinsicHeight * (scaleFactor/10);
        }
        else
        {
            zoomIn = true;
            scaleFactor *= 3;
            //when zoomed out we want to zoom in so we multiply
            xPosition = xPosition - xRatio * imageIntrinsicWidth * (scaleFactor*10);
            yPosition = yPosition - yRatio * imageIntrinsicHeight * (scaleFactor*10);
        }

        xPosition = xPosition - xRatio * imageIntrinsicWidth * scaleFactor;
        yPosition = yPosition - yRatio * imageIntrinsicHeight * scaleFactor;
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        public boolean onDoubleTap(MotionEvent e)
        {
            ...
            float x = e.getX();
            float y = e.getY();
            //we have to see if we are inside the image
            float left = xPosition;
            float top = yPosition;
            float right = left + imageIntrinsicWidth * scaleFactor;
            float bottom = top + imageIntrinsicHeight * scaleFactor;
            ...
            if(inside)
            {
               //we only double tap when we are inside the image
                if(zoomIn)
                {
                    zoomIn = false;
                    scaleFactor /=  3;
                    //when zoomed in we want to decrease the image on the next double tap
                    xPosition = x - xRatio / imageIntrinsicWidth * scaleFactor;
                    yPosition = y - yRatio / imageIntrinsicHeight * scaleFactor;
                }
                else
                {
                    zoomIn = true;
            ...
            //does not let the image get too small or too big
            scaleFactor = Math.max(0.1f, Math.min(scaleFactor, 10f));
